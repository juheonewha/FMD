...
from utils_clean import get_dataset, get_network, get_daparam,epoch_new, TensorDataset, epoch, ParamDiffAug
import copy
...
def main(args):
    ...
    for i in range(num_classes):
        weight_list.append(1-(len(indices_class[i])/labels_all.shape[0]))

    weight_list=torch.tensor(np.array(weight_list),dtype=torch.float32)
    
    criterion = nn.CrossEntropyLoss(weight=weight_list).to(args.device)
    trajectories = []
    ...
    for it in range(0, args.num_experts):
        ...
        teacher_net.train()
        lr = args.lr_teacher
        teacher_optim = torch.optim.SGD(teacher_net.parameters(), lr=lr, momentum=0.9, weight_decay=0.001)  # optimizer_img for synthetic data
        ...
        for e in range(args.train_epochs):
            ...
            train_loss, train_acc = epoch_new("train", dataloader=trainloader, net=teacher_net, optimizer=teacher_optim,
                                        criterion=criterion, args=args, aug=True)

            test_loss, test_acc = epoch_new("test", dataloader=testloader, net=teacher_net, optimizer=None,
            ...
            if e in lr_schedule and args.decay:
                lr *= 0.1
                teacher_optim = torch.optim.SGD(teacher_net.parameters(), lr=lr, momentum=args.mom, weight_decay=args.weight_decay)
                teacher_optim.zero_grad()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--zca', action='store_false')
    parser.add_argument('--decay', action='store_true')
    parser.add_argument('--mom', type=float, default=0, help='momentum')
    parser.add_argument('--weight_decay', type=float, default=0, help='momentum')
